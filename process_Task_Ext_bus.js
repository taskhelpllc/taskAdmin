...
function  processTask_Ext_bus(taskownersId, taskNumber, tasksInProject, taskhelpersId, taskExt3Respn, taskExt3count, phase, s4Aprvtask, taskStatusID){
...
var url = "mongodb+srv://xxxxxxxxxxxxxxxxxxxxxxxxxxxx.mongodb.net/?retryWrites=true&w=majority";
  ...
  var newvalues = { $set: {taskExt3count : taskExt3count}};
...
MongoClient.connect(url, function(err, db) {
  if (err) throw err;
  var dbo = db.db("taskAdmin");
  dbo.collection("taskAdmintable").find({phase: 2, s4Aprvtask: 0, taskStatusID: 4}, { projection: { _id: 0, taskownersId: 1, taskID: 1, tasksInProject: 1, taskhelpersId: 1, taskExt3Respn: 1, taskExt3count: 1, phase: 1, s4Aprvtask: 1, taskStatusID: 1  } }).toArray(function(err, result) {
    if (err) throw err;
    
    result.forEach(function (result) {
    processTask_Ext_bus(result.taskownersId, result.taskID, result.tasksInProject, result.taskhelpersId, result.taskExt3Respn, result.taskExt3count, result.phase, result.s4Aprvtask, result.taskStatusID );
});
