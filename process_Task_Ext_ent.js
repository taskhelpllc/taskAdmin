...
function processTask_Ext_ent(taskownersId, taskNumber, tasksInProject, taskhelpersId, taskExt4Respn, taskExt4count, phase, s4Aprvtask, taskStatusID){
...
var url = "mongodb+srv://xxxxxxxxxxxxxxxxxxxxxxxxxxxx.mongodb.net/?retryWrites=true&w=majority";
  ...
  var newvalues = { $set: {taskExt4count : taskExt4count}};
...
MongoClient.connect(url, function(err, db) {
  if (err) throw err;
  var dbo = db.db("taskAdmin");
  dbo.collection("taskAdmintable").find({phase: 2, s4Aprvtask: 0, taskStatusID: 5}, { projection: { _id: 0, taskownersId: 1, taskID: 1, tasksInProject: 1, taskhelpersId: 1, taskExt4Respn: 1, taskExt4count: 1, phase: 1, s4Aprvtask: 1, taskStatusID: 1  } }).toArray(function(err, result) {
    if (err) throw err;
    
    result.forEach(function (result) {
    processTask_Ext_ent(result.taskownersId, result.taskID, result.tasksInProject, result.taskhelpersId, result.taskExt4Respn, result.taskExt4count, result.phase, result.s4Aprvtask, result.taskStatusID );
});
